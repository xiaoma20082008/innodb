    ...
    trx_t *trx) /* in: transaction */
{
  ...
  if (trx->insert_undo != NULL || trx->update_undo != NULL) {
    ...
    if (undo) {
      ...
      if (!undo->del_marks && (undo->size == 1) && (UT_LIST_GET_LEN(trx_sys->view_list) == 1)) {
        ...
        lsn = trx_undo_update_cleanup_by_discard(trx, &mtr);
    ...
    que_thr_t *thr) /* in: query thread */
{
  ...
  if (node->state == COMMIT_NODE_SEND) {
    ...
    success = trx_sig_send(thr_get_trx(thr), TRX_SIG_COMMIT, TRX_SIG_SELF, TRUE, thr, NULL, &next_thr);
...
void trx_print(
    ...
    trx_t *trx) /* in: transaction */
{
  printf("TRANSACTION %lu %lu, OS thread id %lu", ut_dulint_get_high(trx->id), ut_dulint_get_low(trx->id),
  ...
  case TRX_COMMITTED_IN_MEMORY:
    printf(", committed in memory");
    break;
  default:
    printf(" state %lu", trx->conc_state);
  }

  switch (trx->que_state) {
  case TRX_QUE_RUNNING:
    printf(", runs or sleeps");
    break;
  ...
  case TRX_QUE_COMMITTING:
    ...
    break;
  default:
    printf(" que state %lu", trx->que_state);
  }

  if (0 < UT_LIST_GET_LEN(trx->trx_locks)) {
    printf(", has %lu lock struct(s)", UT_LIST_GET_LEN(trx->trx_locks));
  }
  ...
  if (ut_dulint_cmp(trx->undo_no, ut_dulint_zero) != 0) {
    printf(", undo log entries %lu", ut_dulint_get_low(trx->undo_no));
  }
