...
static ibool lock_deadlock_recursive(
    /*====================*/
    /* out: TRUE if a deadlock was detected
...
operations. */
UNIV_INLINE
void lock_mutex_enter_kernel(void)
/*=========================*/
{
...
operations. */
UNIV_INLINE
void lock_mutex_exit_kernel(void)
/*=========================*/
{
...
mutex_t *lock_rec_get_mutex_for_addr(
    /*========================*/
    ulint space,   /* in: space id */
    ulint page_no) /* in: page number */
{
  return (hash_get_mutex(lock_sys->rec_hash, lock_rec_fold(space, page_no)));
}
...
UNIV_INLINE
void lock_rec_mutex_enter_all(void)
/*==========================*/
{
  hash_table_t *table;
  ulint n_mutexes;
...
UNIV_INLINE
void lock_rec_mutex_exit_all(void)
/*=========================*/
{
  hash_table_t *table;
  ulint n_mutexes;
...
locks. */
UNIV_INLINE
ibool lock_rec_mutex_own_all(void)
/*========================*/
    ...
    trx_t *trx) /* in: transaction */
{
  ...
  while (lock) {
    if (lock->trx == trx && lock_mode_stronger_or_eq(lock_get_mode(lock), mode) && !lock_get_wait(lock) &&
...
UNIV_INLINE
lock_t *lock_rec_other_has_expl_req(
    /*========================*/
    /* out: lock or NULL */
    ulint mode, /* in: lock mode */
    ...
    que_thr_t *thr)      /* in: query thread */
{
  ...
  if (lock->trx != thr_get_trx(thr) || lock->type_mode != (mode | LOCK_REC) || lock_rec_get_n_bits(lock) <= heap_no) {
    return (FALSE);
  }
    ...
    que_thr_t *thr)      /* in: query thread */
{
  ...
  if (lock_rec_has_expl(mode, rec, trx)) {
    ...
    err = DB_SUCCESS;
  } else if (lock_rec_other_has_expl_req(confl_mode, 0, LOCK_WAIT, rec, trx)) {
...
void lock_rec_dequeue_from_page(
    /*=======================*/
    ...
    page_t *old_page) /* in: copy of the old, not reorganized page */
{
  ...
  while (lock) {
    ...
    for (;;) {
      ut_ad(0 ==
            ut_memcmp(page_cur_get_rec(&cur1), page_cur_get_rec(&cur2), rec_get_data_size(page_cur_get_rec(&cur2))));
...
ulint lock_table_enqueue_waiting(
    /*=======================*/
    /* out: DB_LOCK_WAIT, DB_DEADLOCK, or
    DB_QUE_THR_SUSPENDED */
    ...
    lock_t *lock) /* in: table type lock */
{
  ...
  printf("TABLE LOCK table %s trx id %lu %lu", lock->un_member.tab_lock.table->name, (lock->trx)->id.high,
         (lock->trx)->id.low);
    ...
    lock_t *lock) /* in: record type lock */
{
  ...
  printf("RECORD LOCKS space id %lu page no %lu n bits %lu", space, page_no, lock_rec_get_n_bits(lock));

  printf(" table %s index %s trx id %lu %lu", lock->index->table->name, lock->index->name, (lock->trx)->id.high,
         (lock->trx)->id.low);
  ...
  for (i = 0; i < lock_rec_get_n_bits(lock); i++) {
    ...
    if (lock_rec_get_nth_bit(lock, i)) {
      ...
      if (page) {
        len = rec_sprintf(buf, 190, page_find_rec_with_heap_no(page, i));
        buf[len] = '\0';
        printf("%s", buf);
      }
    ...
    if (count >= 3) {
      printf("3 LOCKS PRINTED FOR THIS TRX AND PAGE: SUPPRESSING FURTHER PRINTS\n");
      goto end_prints;
    }
  }
end_prints:
...
/*=================*/
{
  ...
  printf("Trx id counter %lu %lu\n", ut_dulint_get_high(trx_sys->max_trx_id), ut_dulint_get_low(trx_sys->max_trx_id));

  printf("Purge done for trx's n:o < %lu %lu undo n:o < %lu %lu\n", ut_dulint_get_high(purge_sys->purge_trx_no),
         ut_dulint_get_low(purge_sys->purge_trx_no), ut_dulint_get_high(purge_sys->purge_undo_no),
...
loop:
  ...
  if (nth_lock == 0) {
    ...
    if (trx->read_view) {
      printf("Trx read view will not see trx with id >= %lu %lu, sees < %lu %lu\n",
             ut_dulint_get_high(trx->read_view->low_limit_id), ut_dulint_get_low(trx->read_view->low_limit_id),
             ut_dulint_get_high(trx->read_view->up_limit_id), ut_dulint_get_low(trx->read_view->up_limit_id));
    }

    if (trx->que_state == TRX_QUE_LOCK_WAIT) {
      printf("------------------TRX IS WAITING FOR THE LOCK:\n");
  ...
  if (nth_lock >= 10) {
    printf("10 LOCKS PRINTED FOR THIS TRX: SUPPRESSING FURTHER PRINTS\n");
                         ...
                         locks from the successor record */
{
  ...
  *inherit = TRUE;
  ...
  if (lock_rec_other_has_expl_req(LOCK_S, LOCK_GAP, LOCK_WAIT, next_rec, trx)) {
    err = lock_rec_enqueue_waiting(LOCK_X | LOCK_GAP, next_rec, index, thr);
  } else {
    err = DB_SUCCESS;
  }
    ...
    dict_index_t *index) /* in: index of record */
{
  ...
  if (impl_trx) {
    ...
    if (!lock_rec_has_expl(LOCK_X, rec, impl_trx)) {
      ...
      lock_rec_add_to_queue(LOCK_REC | LOCK_X, rec, index, impl_trx);
    }
  }
}
