    ...
    mtr_t *mtr)        /* in: mtr */
{
  ...
  btr_cur_search_to_nth_level(UT_LIST_GET_FIRST(tree->tree_indexes), btr_page_get_level(page, mtr) + 1, tuple,
                    ...
                    be first */
{
  ...
  if ((page_header_get_ptr(page, PAGE_LAST_INSERT) == insert_point) &&
    ...
    if ((page_rec_get_next(insert_point) != supremum) &&
        (page_rec_get_next(page_rec_get_next(insert_point)) != supremum) &&
    ...
    dict_tree_t *tree) /* in: index tree */
{
  ...
  if (tree->type & DICT_IBUF) {
    printf("Sorry, cannot print info of an ibuf tree: use ibuf functions\n");
...
the index. */
static ibool btr_index_rec_validate(
    /*====================*/
    /* out: TRUE if ok */
    ...
    dict_index_t *index) /* in: index */
{
  ...
  for (i = 0; i < n; i++) {
    ...
    if (len != UNIV_SQL_NULL && dtype_is_fixed_size(type) && len != dtype_get_fixed_size(type)) {
      fprintf(stderr, "Record field %lu len is %lu, should be %lu\n", i, len, dtype_get_fixed_size(type));
...
loop:
  ...
  if (level == 0) {
    if (!btr_index_page_validate(page, index)) {
      fprintf(stderr, "Error in page %lu in index %s, level %lu\n", buf_frame_get_page_no(page), index->name, level);
      ret = FALSE;
    }
  }
  ...
  if (right_page_no != FIL_NULL) {
    ...
    if (cmp_rec_rec(page_rec_get_prev(page_get_supremum_rec(page)), page_rec_get_next(page_get_infimum_rec(right_page)),
                    UT_LIST_GET_FIRST(tree->tree_indexes)) >= 0) {

      fprintf(stderr, "InnoDB: Error on pages %lu and %lu in index %s\n", buf_frame_get_page_no(page), right_page_no,
              index->name);

      fprintf(stderr, "InnoDB: records in wrong order on adjacent pages\n");

      rec_sprintf(err_buf, 900, page_rec_get_prev(page_get_supremum_rec(page)));
      fprintf(stderr, "InnoDB: record %s\n", err_buf);

      rec_sprintf(err_buf, 900, page_rec_get_next(page_get_infimum_rec(right_page)));
      fprintf(stderr, "InnoDB: record %s\n", err_buf);
  ...
  if (buf_frame_get_page_no(page) != dict_tree_get_page(tree)) {
    ...
    if (btr_node_ptr_get_child_page_no(node_ptr) != buf_frame_get_page_no(page) ||
      ...
      fprintf(stderr, "InnoDB: node pointer to the page is wrong\n");

      rec_sprintf(err_buf, 900, node_ptr);

      fprintf(stderr, "InnoDB: node ptr %s\n", err_buf);

      fprintf(stderr, "InnoDB: node ptr child page n:o %lu\n", btr_node_ptr_get_child_page_no(node_ptr));

      rec_sprintf(err_buf, 900,
                  btr_page_get_father_for_rec(tree, page, page_rec_get_prev(page_get_supremum_rec(page)), &mtr));
    ...
    if (btr_page_get_level(page, &mtr) > 0) {
      ...
      if (cmp_dtuple_rec(node_ptr_tuple, node_ptr) != 0) {
        ...
        fprintf(stderr, "InnoDB: Error: node ptrs differ on levels > 0\n");

        rec_sprintf(err_buf, 900, node_ptr);

        fprintf(stderr, "InnoDB: node ptr %s\n", err_buf);
        rec_sprintf(err_buf, 900, page_rec_get_next(page_get_infimum_rec(page)));
    ...
    if (right_page_no != FIL_NULL) {
      ...
      if (page_rec_get_next(node_ptr) != page_get_supremum_rec(father_page)) {
        ...
        if (buf_frame_get_page_no(right_father_page) != btr_page_get_next(father_page, &mtr)) {
          ...
          ret = FALSE;
          fprintf(stderr, "InnoDB: node pointer 3 to the right page is wrong\n");
