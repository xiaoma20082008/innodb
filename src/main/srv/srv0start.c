...
static void srv_normalize_path_for_win(
    /*=======================*/
  ...
  for (i = 0; i < srv_n_data_files; i++) {
    ...
    if (ret == FALSE) {
      ...
      if (srv_data_file_is_raw_partition[i] != SRV_OLD_RAW) {
        ...
        if (size != srv_calc_low32(srv_data_file_sizes[i]) || size_high != srv_calc_high32(srv_data_file_sizes[i])) {
      ...
      fprintf(stderr, "  InnoDB: Setting file %s size to %lu MB\n", name,
              (srv_data_file_sizes[i] >> (20 - UNIV_PAGE_SIZE_SHIFT)));
...
static ulint test_measure_cont(
    /*==============*/
    void *arg) {
  ulint i, j;
...
/* out: DB_SUCCESS or error code */
{
  ...
  */
...
#ifdef __WIN__
  if (os_get_os_version() == OS_WIN95 || os_get_os_version() == OS_WIN31) {
  ...
  if (!os_aio_use_native_aio) {
    os_aio_init(4 * SRV_N_PENDING_IOS_PER_THREAD * srv_n_file_io_threads, srv_n_file_io_threads,
  ...
  err = open_or_create_data_files(&create_new_db, &min_flushed_lsn, &min_arch_log_no, &max_flushed_lsn,
                                  &max_arch_log_no, &sum_of_new_sizes);
  if (err != DB_SUCCESS) {
  ...
  for (k = 0; k < srv_n_log_groups; k++) {
    ...
    for (i = 0; i < srv_n_log_files; i++) {
      ...
      if ((log_opened && create_new_db) || (log_opened && log_created)) {
        fprintf(stderr, "InnoDB: Error: all log files must be created at the same time.\n"
  ...
  if (log_created && !create_new_db && !srv_archive_recovery) {
    ...
    if (ut_dulint_cmp(max_flushed_lsn, min_flushed_lsn) != 0 || max_arch_log_no != min_arch_log_no) {
      fprintf(stderr, "InnoDB: Cannot initialize created log files because\n"
    ...
    recv_reset_logs(ut_dulint_align_down(max_flushed_lsn, OS_FILE_LOG_BLOCK_SIZE), max_arch_log_no + 1, TRUE);
