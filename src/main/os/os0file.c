...
ulint os_file_get_last_error(void)
/*========================*/
...
try_again:
  ...
  file = CreateFile(name, GENERIC_READ | GENERIC_WRITE, /* read and write
                                                          access */
...
ibool os_aio(
  ...
  ut_ad((ulint)buf % OS_FILE_LOG_BLOCK_SIZE == 0) ut_ad(offset % OS_FILE_LOG_BLOCK_SIZE == 0);
  ...
  if (os_aio_use_native_aio) {
    if ((ret && len == n) || (!ret && GetLastError() == ERROR_IO_PENDING)) {
      ...
      if (mode == OS_AIO_SYNC) {
        ...
        return (os_aio_windows_handle(ULINT_UNDEFINED, slot->pos, &dummy_mess1, &dummy_mess2, &dummy_type));
      }
    ...
    void **message2, ulint *type) /* out: OS_FILE_WRITE or ..._READ */
{
  ...
  if (array == os_aio_sync_array) {
    ...
    srv_io_thread_op_info[orig_seg] = "wait windows aio for n pages";
