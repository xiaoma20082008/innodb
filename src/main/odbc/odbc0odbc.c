...
UNIV_INLINE
odbc_param_t *stat_get_nth_param(
    /*===============*/
    /* out: nth parameter */
                       ...
                       header of size SESS_CLI_MSG_DATA) */
{
  ...
  ret = com_sendto(conn->com_endpoint, msg, SESS_CLI_MSG_DATA + len, conn->server_name, conn->server_name_len);
  ut_a(ret == 0);
}

/**************************************************************************
Receives a message in datagram_buf from the server. */
static void odbc_recv_srv_msg(
    /*==============*/
    odbc_conn_t *conn, /* in: connection, does not have to be
                       connected yet */
                       ...
                       standard header of size SESS_SRV_MSG_DATA) */
{
  ...
  ret = com_recvfrom(conn->com_endpoint, conn->in_datagram_buf, ODBC_DATAGRAM_SIZE, &total_len, (char *)conn->addr_buf,
    ...
    SWORD cbAuthStr)  /* in: password length */
{
  ...
  err = com_endpoint_set_option(ep, COM_OPT_MAX_DGRAM_SIZE, (byte *)&size, 4);
  ut_a(err == 0);
  ...
  err = com_bind(ep, (char *)catenated_name, (ulint)cbDSN + (ulint)cbUID);
  ut_a(err == 0);
  ...
  ut_a(len == 0);
  ut_a(sess_srv_msg_get_type(conn->in_datagram_buf) == SESS_SRV_ACCEPT_CONNECT);

  conn->sess_id = mach_read_from_8(conn->in_datagram_buf + SESS_SRV_MSG_SESS_ID);
...
static void odbc_stat_store_error_msg(
    /*======================*/
    ...
    SDWORD cbSqlStr) /* in: SQL string length */
{
  ...
  ut_memcpy(conn->out_datagram_buf + SESS_CLI_MSG_DATA, szSqlStr, 1 + (ulint)cbSqlStr);
  ...
  if (stat->n_params > 0) {
    ...
    stat->params = mem_alloc(stat->n_params * sizeof(odbc_param_t));
    for (i = 0; i < stat->n_params; i++) {
      param = stat_get_nth_param(stat, i);

      param->is_input = mach_read_from_1(msg + SESS_SRV_MSG_DATA + 8 + i);
