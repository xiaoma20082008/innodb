...
#ifdef UNIV_SEARCH_DEBUG
  page_cur_search_with_match(page, tuple, PAGE_CUR_DBG, iup_matched_fields, iup_matched_bytes, ilow_matched_fields,
                             ilow_matched_bytes, &cursor2);
    ...
    page_cur_t *cursor) /* out: page cursor */
{
  ...
  if ((page_header_get_field(page, PAGE_LEVEL) == 0) && (mode == PAGE_CUR_LE) &&
    ...
    if (page_cur_try_search_shortcut(page, tuple, iup_matched_fields, iup_matched_bytes, ilow_matched_fields,
...
/*#endif */
  ...
  dbg_cmp = page_cmp_dtuple_rec_with_match(tuple, low_rec, &dbg_matched_fields, &dbg_matched_bytes);
  if (mode == PAGE_CUR_G) {
    ut_a(dbg_cmp >= 0);
  ...
  dbg_cmp = page_cmp_dtuple_rec_with_match(tuple, up_rec, &dbg_matched_fields, &dbg_matched_bytes);
  if (mode == PAGE_CUR_G) {
    ut_a(dbg_cmp == -1);
  } else if (mode == PAGE_CUR_GE) {
    ut_a(dbg_cmp <= 0);
