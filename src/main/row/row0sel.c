    ...
    dict_index_t *clust_index) /* in: clustered index */
{
  ...
  for (i = 0; i < n; i++) {
    col = dict_field_get_col(dict_index_get_nth_field(sec_index, i));

    clust_field = rec_get_nth_field(clust_rec, dict_col_get_clust_pos(col), &clust_len);
    ...
    mtr_t *mtr)             /* in: mtr */
{
  ...
  err = row_vers_build_for_consistent_read(rec, mtr, plan->index, read_view, plan->old_vers_heap, old_vers);
  return (err);
}
                      ...
                      access the clustered index */
{
  ...
  if (!node->read_view) {
    ...
    err = lock_clust_rec_read_check_and_lock(0, clust_rec, index, node->row_lock_mode, thr);
    if (err != DB_SUCCESS) {
    ...
    if (!lock_clust_rec_cons_read_sees(clust_rec, index, node->read_view)) {

      err = row_sel_build_prev_vers(node->read_view, plan, clust_rec, &old_vers, mtr);
      if (err != DB_SUCCESS) {
  ...
  row_sel_fetch_columns(index, clust_rec, UT_LIST_GET_FIRST(plan->columns));
  *out_rec = clust_rec;
    ...
    que_thr_t *thr)      /* in: query thread */
{
  ...
  if (index->type & DICT_CLUSTERED) {
    err = lock_clust_rec_read_check_and_lock(0, rec, index, mode, thr);
  } else {
    err = lock_sec_rec_read_check_and_lock(0, rec, index, mode, thr);
  }
    ...
    mtr_t *mtr)       /* in: mtr */
{
  ...
  if (index->type & DICT_CLUSTERED) {
    if (!lock_clust_rec_cons_read_sees(rec, index, node->read_view)) {
      return (SEL_RETRY);
    }
...
rec_loop:
  ...
  if (!node->asc && cursor_just_opened && (rec != page_get_supremum_rec(buf_frame_align(rec)))) {
    ...
    if (!consistent_read) {
      err = sel_set_rec_lock(page_rec_get_next(rec), index, node->row_lock_mode, thr);
      if (err != DB_SUCCESS) {
  ...
  if (consistent_read) {
    ...
    if (index->type & DICT_CLUSTERED) {
      ...
      if (!lock_clust_rec_cons_read_sees(rec, index, node->read_view)) {
        ...
        err = row_sel_build_prev_vers(node->read_view, plan, rec, &old_vers, &mtr);
        if (err != DB_SUCCESS) {
    ...
    ulint key_len)       /* in: MySQL key value length */
{
  ...
  while (key_ptr < key_end) {
    ...
    row_mysql_store_col_in_innobase_format(dfield, buf, key_ptr + offset, len, dfield_get_type(dfield)->mtype,
                                           dfield_get_type(dfield)->prtype & DATA_UNSIGNED);
  next_part:
    key_ptr += (offset + len);
    ...
    if (key_ptr > key_end) {
      ...
      dfield_set_data(dfield, buf, len - (ulint)(key_ptr - key_end));
    }
    ...
    dict_index_t *index)      /* in: index of the record */
{
  ...
  data = rec_get_nth_field(index_rec, dict_index_get_sys_col_pos(index, DATA_ROW_ID), &len);
...
static void row_sel_store_mysql_rec(
    /*====================*/
                              ...
                              template */
{
  ...
  for (i = 0; i < prebuilt->n_template; i++) {
    ...
    if (rec_get_nth_field_extern_bit(rec, templ->rec_field_no)) {
      ...
      data = btr_rec_copy_externally_stored_field(rec, templ->rec_field_no, &len, extern_field_heap);
    ...
    if (len != UNIV_SQL_NULL) {
      if (templ->type == DATA_BLOB) {
        ...
        blob_buf = mem_heap_alloc(prebuilt->blob_heap, len);
        ut_memcpy(blob_buf, data, len);

        data = blob_buf;
      }

      row_sel_field_store_in_mysql_format(mysql_rec + templ->mysql_col_offset, templ->mysql_col_len, data, len,
    ...
    mtr_t *mtr)                /* in: mtr */
{
  ...
  err = row_vers_build_for_consistent_read(rec, mtr, clust_index, read_view, prebuilt->old_vers_heap, old_vers);
  return (err);
}
                              ...
                              access the clustered index */
{
  ...
  *out_rec = NULL;
  ...
  if (prebuilt->select_lock_type != LOCK_NONE) {
    ...
    err = lock_clust_rec_read_check_and_lock(0, clust_rec, clust_index, prebuilt->select_lock_type, thr);
    if (err != DB_SUCCESS) {
    ...
    if (!lock_clust_rec_cons_read_sees(clust_rec, clust_index, trx->read_view)) {

      err = row_sel_build_prev_vers_for_mysql(trx->read_view, clust_index, prebuilt, clust_rec, &old_vers, mtr);
    ...
    row_prebuilt_t *prebuilt) /* in: prebuilt struct */
{
  ...
  ut_memcpy(buf, prebuilt->fetch_cache[prebuilt->fetch_cache_first], prebuilt->mysql_row_len);
    ...
    rec_t *rec)               /* in: record to push */
{
  ...
  if (prebuilt->fetch_cache[0] == NULL) {
    ...
    for (i = 0; i < MYSQL_FETCH_CACHE_SIZE; i++) {
      prebuilt->fetch_cache[i] = mem_alloc(prebuilt->mysql_row_len);
    }
  }

  ut_ad(prebuilt->fetch_cache_first == 0);

  row_sel_store_mysql_rec(prebuilt->fetch_cache[prebuilt->n_fetch_cached], prebuilt, rec);
                              ...
                              cursor 'direction' should be 0. */
{
  ...
  */
  ...
  if (match_mode == ROW_SEL_EXACT && index->type & DICT_UNIQUE && index->type & DICT_CLUSTERED &&
    ...
    if (trx->mysql_n_tables_locked == 0 && !prebuilt->sql_stat_start) {
      ...
      shortcut = row_sel_try_search_shortcut_for_mysql(&rec, prebuilt, &mtr);
...
no_shortcut:
  ...
  if (direction != 0) {
    moved = sel_restore_position_for_mysql(BTR_SEARCH_LEAF, pcur, moves_up, &mtr);
    if (!moved) {
      goto next_rec;
    }
...
rec_loop:
  ...
  */
  ...
  if (prebuilt->select_lock_type != LOCK_NONE) {
    ...
    err = sel_set_rec_lock(rec, index, prebuilt->select_lock_type, thr);
    if (err != DB_SUCCESS) {
  ...
  if (unique_search_from_clust_index && btr_pcur_get_up_match(pcur) == dtuple_get_n_fields(search_tuple)) {
  ...
  if (prebuilt->select_lock_type == LOCK_NONE) {
    ...
    if (index == clust_index) {
      ...
      if (!lock_clust_rec_cons_read_sees(rec, index, trx->read_view)) {
        ...
        err = row_sel_build_prev_vers_for_mysql(trx->read_view, clust_index, prebuilt, rec, &old_vers, &mtr);
  ...
  if (index != clust_index && (cons_read_requires_clust_rec || prebuilt->need_to_access_clustered)) {
    ...
    err = row_sel_get_clust_rec_for_mysql(prebuilt, index, rec, thr, &clust_rec, &mtr);
    if (err != DB_SUCCESS) {
  ...
  if (prebuilt->n_rows_fetched >= MYSQL_FETCH_CACHE_THRESHOLD && !prebuilt->templ_contains_blob &&
    ...
    if (prebuilt->template_type == ROW_MYSQL_DUMMY_TEMPLATE) {
      ut_memcpy(buf + 4, rec - rec_get_extra_size(rec), rec_get_size(rec));
    ...
    if (prebuilt->clust_index_was_generated) {
      row_sel_store_row_id_to_prebuilt(prebuilt, index_rec, index);
    }
  }
got_row:
  ...
  if (!unique_search_from_clust_index || prebuilt->select_lock_type == LOCK_X) {
...
lock_wait_or_error:
  ...
  if (was_lock_wait) {
    ...
    sel_restore_position_for_mysql(BTR_SEARCH_LEAF, pcur, moves_up, &mtr);
    mode = pcur->search_mode;
