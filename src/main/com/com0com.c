...
UNIV_INLINE
void com_endpoint_set_type(
    /*==================*/
    com_endpoint_t *ep, ulint type) {
  ut_ad(ep);
  ut_ad(type == COM_SHM);
...
UNIV_INLINE
void com_endpoint_set_par(
    /*=================*/
    com_endpoint_t *ep, void *par) {
  ut_ad(ep);
  ut_ad(par);
...
UNIV_INLINE
void com_endpoint_set_bound(
    /*===================*/
    com_endpoint_t *ep, ibool bound) {
  ut_ad(ep);
...
com_endpoint_t *com_endpoint_create(
    /*================*/
    ...
    ulint optlen)       /* in: option value buffer length */
{
  ...
  if (type == COM_SHM) {
    ret = com_shm_endpoint_set_option((com_shm_endpoint_t *)par, optno, optval, optlen);
  } else {
    ret = 0;
    ut_error;
  }
    ...
    ulint *addr_len)    /* out: address name length */
{
  ...
  if (type == COM_SHM) {
    ret = com_shm_recvfrom((com_shm_endpoint_t *)par, buf, buf_len, len, from, from_len, addr_len);
  } else {
    ret = 0;
    ...
    ulint tolen)        /* in: address name length */
{
  ...
  if (type == COM_SHM) {
    ret = com_shm_sendto((com_shm_endpoint_t *)par, buf, len, to, tolen);
  } else {
    ret = 0;
    ut_error;
  }
...
ulint com_endpoint_get_max_size(
    /*======================*/
    /* out: maximum size */
    com_endpoint_t *ep) /* in: endpoint */
{
