    ...
    ulint line)      /* in: line where requested */
{
  ...
  } else if ((rw_lock_get_writer(lock) == RW_LOCK_WAIT_EX) && (lock->writer_thread == os_thread_get_curr_id())) {
  ...
  } else if ((rw_lock_get_writer(lock) == RW_LOCK_EX) && (lock->writer_thread == os_thread_get_curr_id()) &&
             (lock->pass == 0) && (pass == 0)) {
...
{
loop:
  if (0 == mutex_enter_nowait(&rw_lock_debug_mutex, IB__FILE__, __LINE__)) {
    return;
  }
    ...
    ulint lock_type) /* in: lock type */
{
  ...
  while (info != NULL) {
    if ((pass == info->pass) && ((pass != 0) || (info->thread_id == os_thread_get_curr_id())) &&
        (info->lock_type == lock_type)) {
    ...
    ulint lock_type) /* in: lock type */
{
  ...
  while (info != NULL) {
    ...
    if ((info->thread_id == os_thread_get_curr_id()) && (info->pass == 0) && (info->lock_type == lock_type)) {
    ...
    rw_lock_debug_t *info) /* in: debug struct */
{
  ...
  printf("Locked: thread %ld file %s line %ld  ", info->thread_id, info->file_name, info->line);
