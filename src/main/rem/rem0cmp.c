    ...
    dtype_t *type2) /* in: type 2 */
{
  ...
  if (type1->mtype == DATA_MYSQL || type1->mtype == DATA_VARMYSQL) {

    if ((type1->prtype & ~DATA_NOT_NULL) != (type2->prtype & ~DATA_NOT_NULL)) {
...
order_resolved:
  ut_ad((ret >= -1) && (ret <= 1));
  ut_ad(ret == cmp_debug_dtuple_rec_with_match(dtuple, rec, matched_fields));
...
ibool cmp_dtuple_rec_prefix_equal(
    /*========================*/
    /* out: TRUE if equal */
    dtuple_t *dtuple, /* in: data tuple */
    rec_t *rec,       /* in: physical record */
