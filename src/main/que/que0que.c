...
static que_thr_t *que_try_parallelize(
    /*================*/
...
client. */
static void que_thr_add_update_info(
    /*====================*/
    que_thr_t *thr) /* in: query thread */
{
  que_fork_t *graph;
  ...
  mach_write_to_8(thr->msg_buf + SESS_SRV_MSG_N_INSERTS, graph->n_inserts);
  mach_write_to_8(thr->msg_buf + SESS_SRV_MSG_N_UPDATES, graph->n_updates);
  mach_write_to_8(thr->msg_buf + SESS_SRV_MSG_N_DELETES, graph->n_deletes);
}
#endif
...
static ulint que_build_srv_msg(
    /*==============*/
...
que_thr_t *que_thr_check_if_switch(
    /*====================*/
