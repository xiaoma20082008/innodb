...
#define DICT_POOL_PER_PROCEDURE_HASH                                                                                   \
  512 /* buffer pool max size per stored                                                                               \
    ...
    trx_t *trx)      /* in: transaction handle */
{
  ...
  if (ut_dulint_cmp(table_id, DICT_FIELDS_ID) <= 0 || trx->dict_operation) {
    ...
    dict_table_t *table) /* in, own: table */
{
  ...
  while (foreign != NULL) {
    ut_a(0 == ut_strcmp(foreign->referenced_table_name, table->name));
  ...
  while (index != NULL) {
    if (dict_index_get_n_fields(index) >= n_cols) {
      ...
      for (i = 0; i < n_cols; i++) {
        ...
        if (ut_strlen(columns[i]) != ut_strlen(col_name) || 0 != ut_memcmp(columns[i], col_name, ut_strlen(col_name))) {
          break;
        }

        if (types_idx &&
            !cmp_types_are_equal(dict_index_get_nth_type(index, i), dict_index_get_nth_type(types_idx, i))) {
    ...
    dict_foreign_t *foreign) /* in, own: foreign key constraint */
{
  ...
  if (for_in_cache->referenced_table == NULL && ref_table) {
    index = dict_foreign_find_index(ref_table, for_in_cache->referenced_col_names, for_in_cache->n_fields,
  ...
  if (for_in_cache->foreign_table == NULL && for_table) {
    index = dict_foreign_find_index(for_table, for_in_cache->foreign_col_names, for_in_cache->n_fields,
  ...
  *success = FALSE;
  ...
  for (i = 0; i < dict_table_get_n_cols(table); i++) {
    ...
    if (ut_strlen(col->name) == (ulint)(ptr - old_ptr) &&
        0 == ut_cmp_in_lower_case(col->name, old_ptr, (ulint)(ptr - old_ptr))) {
      /* Found */
...
static char *dict_scan_table_name(
    /*=================*/
    /* out: scanned to */
    char *ptr,            /* in: scanned to */
...
#ifdef __WIN__
    ut_cpy_in_lower_case(second_table_name + i, old_ptr, ptr - old_ptr);
#else
...
#ifdef __WIN__
    ut_cpy_in_lower_case(second_table_name, old_ptr, ptr - old_ptr);
#else
...
col_loop1:
  ptr = dict_scan_col(ptr, &success, table, columns + i, column_names + i);
  if (!success) {
    return (DB_CANNOT_ADD_CONSTRAINT);
  }
...
col_loop2:
  ptr = dict_scan_col(ptr, &success, referenced_table, columns + i, column_names + i);
  i++;
  ...
  foreign->referenced_table_name = mem_heap_alloc(foreign->heap, 1 + ut_strlen(referenced_table->name));

  ut_memcpy(foreign->referenced_table_name, referenced_table->name, 1 + ut_strlen(referenced_table->name));

  foreign->referenced_col_names = mem_heap_alloc(foreign->heap, i * sizeof(void *));
  for (i = 0; i < foreign->n_fields; i++) {
    foreign->referenced_col_names[i] = mem_heap_alloc(foreign->heap, 1 + ut_strlen(columns[i]->name));
    ut_memcpy(foreign->referenced_col_names[i], columns[i]->name, 1 + ut_strlen(columns[i]->name));
  }
...
void dict_procedure_add_to_cache(
    /*========================*/
    dict_proc_t *proc) /* in: procedure */
{
  ...
  /* Look for a procedure with the same name: error if such exists */
  {
    ...
    HASH_SEARCH(name_hash, dict_sys->procedure_hash, fold, proc2, (ut_strcmp(proc2->name, proc->name) == 0));
    ut_a(proc2 == NULL);
  }
...
que_t *dict_procedure_reserve_parsed_copy(
    /*===============================*/
    /* out: the query graph */
...
void dict_procedure_release_parsed_copy(
    /*===============================*/
    ...
    dict_table_t *table) /* in: table */
{
  ...
  printf("TABLE: name %s, id %lu %lu, columns %lu, indexes %lu, appr.rows %lu\n", table->name,
         ut_dulint_get_high(table->id), ut_dulint_get_low(table->id), table->n_cols, UT_LIST_GET_LEN(table->indexes),
    ...
    dict_index_t *index) /* in: index */
{
  ...
  printf("  INDEX: name %s, table name %s, id %lu %lu, fields %lu/%lu, type %lu\n", index->name, index->table_name,
         ut_dulint_get_high(tree->id), ut_dulint_get_low(tree->id), index->n_user_defined_cols, index->n_fields,
         index->type);
  printf("   root page %lu, appr.key vals %lu, leaf pages %lu, size pages %lu\n", tree->page, (ulint)n_vals,
         index->stat_n_leaf_pages, index->stat_index_size);
