  ...
  while (group) {
    ...
    for (field = LOG_CHECKPOINT_1; field <= LOG_CHECKPOINT_2; field += LOG_CHECKPOINT_2 - LOG_CHECKPOINT_1) {
      ...
      if ((fold & 0xFFFFFFFF) != mach_read_from_4(buf + LOG_CHECKPOINT_CHECKSUM_1)) {
        if (log_debug_writes) {
          fprintf(stderr, "InnoDB: Checkpoint in group %lu at %lu invalid, %lu, %lu\n", group->id, field,
                  fold & 0xFFFFFFFF, mach_read_from_4(buf + LOG_CHECKPOINT_CHECKSUM_1));
        }
      ...
      if ((fold & 0xFFFFFFFF) != mach_read_from_4(buf + LOG_CHECKPOINT_CHECKSUM_2)) {
        if (log_debug_writes) {
          fprintf(stderr, "InnoDB: Checkpoint in group %lu at %lu invalid, %lu, %lu\n", group->id, field,
                  fold & 0xFFFFFFFF, mach_read_from_4(buf + LOG_CHECKPOINT_CHECKSUM_2));
        }
        goto not_consistent;
      }
    ...
    while (recv_addr) {
      ...
      if (recv_addr->state == RECV_NOT_PROCESSED) {
        if (!has_printed) {
          ...
          fprintf(stderr, "  InnoDB: Starting an apply batch of log records to the database...\n"
        ...
        if (buf_page_peek(space, page_no)) {
          ...
          buf_page_dbg_add_level(page, SYNC_NO_ORDER_CHECK);
          recv_recover_page(FALSE, page, space, page_no);
          mtr_commit(&mtr);
        } else {
    ...
    ulint n_pages) /* in: number of pages */
{
  ...
  for (page_no = 0; page_no < n_pages; page_no++) {
    ...
    frame = buf_page_get_gen(space1, page_no, RW_S_LATCH, NULL, BUF_GET_IF_IN_POOL, IB__FILE__, __LINE__, &mtr);
    if (frame) {
...
loop:
  ...
  if (single_rec || *ptr == MLOG_DUMMY_RECORD) {
    ...
    if (2 * n_recs * (sizeof(recv_t) + sizeof(recv_addr_t)) + total_len + mem_heap_get_size(recv_sys->heap) +
    ...
    ut_ad(2 * n_recs * (sizeof(recv_t) + sizeof(recv_addr_t)) + total_len + mem_heap_get_size(recv_sys->heap) +
              RECV_POOL_N_FREE_BLOCKS * UNIV_PAGE_SIZE <
          buf_pool_get_curr_size());
  ...
  if (store_to_hash && buf_get_free_list_len() < RECV_POOL_N_FREE_BLOCKS) {
    ...
    dulint *group_scanned_lsn) /* out: scanning succeeded up to this lsn */
{
  ...
  while (log_block < buf + len && !finished) {
    ...
    if (no != log_block_get_trl_no(log_block) || no != log_block_convert_lsn_to_no(scanned_lsn)) {
  ...
  *group_scanned_lsn = scanned_lsn;
  ...
  if (more_data) {
    ...
    if (recv_scan_print_counter < 10 || (recv_scan_print_counter % 10 == 0)) {
      fprintf(stderr, "InnoDB: Doing recovery: scanned up to log sequence number %lu %lu\n",
              ut_dulint_get_high(*group_scanned_lsn), ut_dulint_get_low(*group_scanned_lsn));
      if (recv_scan_print_counter == 10) {
        fprintf(stderr, "InnoDB: After this prints a line for every 10th scan sweep:\n");
      }
    }
    ...
    dulint *group_scanned_lsn) /* out: scanning succeeded up to this lsn */
{
  ...
  while (!finished) {
    ...
    finished = recv_scan_log_recs(TRUE, log_sys->buf, RECV_SCAN_SIZE, start_lsn, contiguous_lsn, group_scanned_lsn);
    start_lsn = end_lsn;
  }
    ...
    dulint max_flushed_lsn) /* in: max flushed lsn from data files */
{
  ...
  if (type == LOG_CHECKPOINT) {
    ...
    if (ut_dulint_cmp(checkpoint_lsn, max_flushed_lsn) != 0 || ut_dulint_cmp(checkpoint_lsn, min_flushed_lsn) != 0) {
      ...
      fprintf(stderr, "  InnoDB: Database was not shut down normally.\n"
                      "InnoDB: Starting recovery from log files...\n");
      fprintf(stderr,
              "InnoDB: Starting log scan based on checkpoint at\n"
              "InnoDB: log sequence number %lu %lu\n",
              ut_dulint_get_high(checkpoint_lsn), ut_dulint_get_low(checkpoint_lsn));
    }
  }
...
try_open_again:
  ...
  for (;;) {
    ...
    ret = recv_scan_log_recs(TRUE, buf, len, start_lsn, &dummy_lsn, &scanned_lsn);
