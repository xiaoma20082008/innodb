    ...
    byte *page) /* in: buffer page */
{
  ...
  +ut_fold_binary(page + FIL_PAGE_DATA, UNIV_PAGE_SIZE - FIL_PAGE_DATA - FIL_PAGE_END_LSN);
    ...
    byte *read_buf) /* in: a database page */
{
  ...
  fprintf(stderr, "InnoDB: Page checksum %lu stored checksum %lu\n", checksum,
          mach_read_from_4(read_buf + UNIV_PAGE_SIZE - FIL_PAGE_END_LSN));
...
static buf_pool_t *buf_pool_create(
    /*============*/
    ...
    buf_block_t *block) /* in: pointer to the block in question */
{
  ...
  if (io_type == BUF_IO_READ) {
    ...
    if (recv_recovery_is_on()) {
      recv_recover_page(TRUE, block->frame, block->space, block->offset);
    }
...
/*===========*/
{
  ...
  for (i = 0; i < n_found; i++) {
    ...
    printf("Block count for index %lu in buffer is about %lu", ut_dulint_get_low(index_ids[i]), counts[i]);
