...
ulint mem_field_header_get_len(byte *field) { return (mach_read(field - 2 * sizeof(ulint))); }

void mem_field_header_set_check(byte *field, ulint check) { mach_write(field - sizeof(ulint), check); }

ulint mem_field_header_get_check(byte *field) { return (mach_read(field - sizeof(ulint))); }

void mem_field_trailer_set_check(byte *field, ulint check) {
  mach_write(field + mem_field_header_get_len(field), check);
}

ulint mem_field_trailer_get_check(byte *field) { return (mach_read(field + mem_field_header_get_len(field))); }
    ...
    ulint line)       /* in: line where freed */
{
  ...
  if (node == NULL) {
    printf("Memory heap or buffer freed in %s line %lu did not exist.\n", file_name, line);
    ut_error;
  }
...
/* out: TRUE if no heaps exist */
{
  ...
  printf("Sorry, non-debug version cannot check if all memory is freed.\n");
    ...
    byte *ptr) /* in: pointer to place of possible corruption */
{
  ...
  fprintf(stderr, "InnoDB: Apparent memory corruption: mem dump %s\n", srv_fatal_errbuf);

  fprintf(stderr, "InnoDB: Scanning backward trying to find previous allocated mem blocks\n");
