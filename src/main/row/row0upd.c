...
ibool row_upd_changes_field_size(
    /*=======================*/
    ...
    mem_heap_t *heap)    /* in: memory heap from which allocated */
{
  ...
  for (i = 0; i < dtuple_get_n_fields(entry); i++) {
    ...
    if (rec_get_nth_field_extern_bit(rec, i) != upd_ext_vec_contains(ext_vec, n_ext_vec, i) ||
  ...
  for (i = 0; i < n_unique; i++) {
    ...
    for (j = 0; j < n_upd_fields; j++) {
      ...
      if (col_pos == upd_field->field_no &&
          (row == NULL || !dfield_datas_are_binary_equal(dtuple_get_nth_field(row, col_no), &(upd_field->new_val)))) {
        return (TRUE);
      }
    }
  }
    ...
    mtr_t *mtr)          /* in: mtr; gets committed here */
{
  ...
  if (node->state != UPD_NODE_INSERT_CLUSTERED) {
    ...
    btr_cur_mark_extern_inherited_fields(btr_cur_get_rec(btr_cur), node->update, mtr);
    if (check_ref) {
      /* NOTE that the following call loses
      the position of pcur ! */
