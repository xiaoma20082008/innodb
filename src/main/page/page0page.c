    ...
    mtr_t *mtr)         /* in: mini-transaction handle */
{
  ...
  if (page_template) {
    ...
    ut_memcpy(page + UNIV_PAGE_SIZE - PAGE_EMPTY_DIR_START, page_template + UNIV_PAGE_SIZE - PAGE_EMPTY_DIR_START,
    ...
    mtr_t *mtr)       /* in: mtr */
{
  ...
  page_delete_rec_list_end(page, split_rec, new_n_recs - old_n_recs, new_data_size - old_data_size, mtr);
}
...
void page_header_print(
  ...
  printf("Page address %lx, n records %lu\n", (ulint)page, page_header_get_field(page, PAGE_N_RECS));
  ...
  printf("Page n heap %lu, free %lu, garbage %lu\n", page_header_get_field(page, PAGE_N_HEAP),
         page_header_get_field(page, PAGE_FREE), page_header_get_field(page, PAGE_GARBAGE));
                         ...
                         the page record type definition */
{
  ...
  if (!(page_header_get_ptr(page, PAGE_HEAP_TOP) <= page_dir_get_nth_slot(page, n_slots - 1))) {
    fprintf(stderr, "Record heap and dir overlap on a page in index %s, %lu, %lu\n", index->name,
            (ulint)page_header_get_ptr(page, PAGE_HEAP_TOP), (ulint)page_dir_get_nth_slot(page, n_slots - 1));
  ...
  for (;;) {
    ...
    if (rec_get_n_owned(rec) != 0) {
      ...
      if (rec_get_n_owned(rec) != own_count) {
        fprintf(stderr, "Wrong owned count %lu, %lu, in index %s\n", rec_get_n_owned(rec), own_count, index->name);

        goto func_exit;
      }

      if (page_dir_slot_get_rec(slot) != rec) {
        fprintf(stderr, "Dir slot does not point to right rec in %s\n", index->name);
    ...
    if (rec_get_next_offs(rec) < FIL_PAGE_DATA || rec_get_next_offs(rec) >= UNIV_PAGE_SIZE) {
      fprintf(stderr, "Next record offset wrong %lu in index %s\n", rec_get_next_offs(rec), index->name);
