    ...
    sync_cell_t *cell) /* in: sync cell */
{
  ...
  fprintf(file, "--Thread %lu has waited at %s line %lu for %.2f seconds the semaphore:\n", (ulint)cell->thread,
          cell->file, cell->line, difftime(time(NULL), cell->reservation_time));
  ...
  if (type == SYNC_MUTEX) {
    ...
    fprintf(file, "Mutex at %lx created file %s line %lu, lock var %lu\n", (ulint)mutex, mutex->cfile_name,
            mutex->cline, mutex->lock_word);
    fprintf(file, "Last time reserved in file %s line %lu, waiters flag %lu\n", mutex->file_name, mutex->line,
            mutex->waiters);
    ...
    if (rwlock->writer != RW_LOCK_NOT_LOCKED) {
      fprintf(file, "a writer (thread id %lu) has reserved it in mode", (ulint)rwlock->writer_thread);
    ...
    ulint depth)        /* in: recursion depth */
{
  ...
  if (cell->request_type == SYNC_MUTEX) {
    ...
    while (debug != NULL) {
      ...
      if (((debug->lock_type == RW_LOCK_EX) && (thread != cell->thread)) ||
          ((debug->lock_type == RW_LOCK_WAIT_EX) && (thread != cell->thread)) || (debug->lock_type == RW_LOCK_SHARED)) {
...
void sync_array_print_long_waits(void)
/*=============================*/
{
  sync_cell_t *cell;
  ibool old_val;
  ...
  for (i = 0; i < sync_primary_wait_array->n_cells; i++) {
    ...
    if (cell->wait_object != NULL && difftime(time(NULL), cell->reservation_time) > 420) {

      fprintf(stderr, "InnoDB: Error: semaphore wait has lasted > 420 seconds\n"
