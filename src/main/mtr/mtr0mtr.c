...
static void mtr_log_write_full_page(
    /*====================*/
...
void mtr_log_write_backup_full_pages(
    /*============================*/
...
static ibool mtr_first_to_modify_page_after_backup(
    /*==================================*/
  ...
  while (offset > 0) {
    ...
    if ((block != NULL) && (type == MTR_MEMO_PAGE_X_FIX)) {
      ...
      *n_pages = *n_pages + 1;
      ...
      if (ut_dulint_cmp(buf_frame_get_newest_modification(block->frame), backup_lsn) <= 0) {

        printf("Page %lu newest %lu backup %lu\n", block->offset,
               ut_dulint_get_low(buf_frame_get_newest_modification(block->frame)), ut_dulint_get_low(backup_lsn));
