    ...
    mtr_t *mtr)             /* in: mtr */
{
  ...
  if (btr_search_latch.writer != RW_LOCK_NOT_LOCKED && latch_mode <= BTR_MODIFY_LEAF && info->last_hash_succ &&
      !estimate && btr_search_guess_on_hash(index, info, tuple, mode, latch_mode, cursor, has_search_latch, mtr)) {
  ...
  retry_page_get:
    ...
    if (page == NULL) {
      ...
      if (ibuf_should_try(index) && ibuf_insert(tuple, index, space, page_no, cursor->thr)) {
...
ulint btr_cur_update_sec_rec_in_place(
    /*============================*/
    /* out: DB_SUCCESS or error number */
...
static void btr_cur_copy_new_col_vals(
    /*======================*/
    ...
    mtr_t *mtr)                 /* in: mtr */
{
  ...
  btr_rec_free_externally_stored_fields(cursor->index, btr_cur_get_rec(cursor), in_rollback, mtr);
  ...
  if ((btr_page_get_level(page, mtr) > 0) && (page_rec_get_next(page_get_infimum_rec(page)) == rec)) {
    ...
    if (btr_page_get_prev(page, mtr) == FIL_NULL) {
      ...
      node_ptr = dict_tree_build_node_ptr(tree, page_rec_get_next(rec), buf_frame_get_page_no(page), heap,
    ...
    dict_index_t *index) /* in: index */
{
  ...
  for (j = 0; j <= n_cols; j++) {
    index->stat_n_diff_key_vals[j] = (n_diff[j] * index->stat_n_leaf_pages + BTR_KEY_VAL_ESTIMATE_N_PAGES - 1 +
...
void btr_cur_unmark_extern_fields(
    /*=========================*/
    rec_t *rec, /* in: record in a clustered index */
    mtr_t *mtr) /* in: mtr */
{
