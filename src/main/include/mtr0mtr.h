...
#define MLOG_REC_CLUST_DELETE_MARK                                                                                     \
  ((byte)10) /* mark clustered index record deleted */
#define MLOG_REC_SEC_DELETE_MARK                                                                                       \
  ((byte)11) /* mark secondary index record deleted */
#define MLOG_REC_UPDATE_IN_PLACE                                                                                       \
  ((byte)13) /* update of a record,preserves record field sizes */
#define MLOG_REC_DELETE                                                                                                \
  ((byte)14) /* delete a record from a page */
#define MLOG_LIST_END_DELETE                                                                                           \
  ((byte)15) /* delete record list end on index page */
#define MLOG_LIST_START_DELETE                                                                                         \
  ((byte)16) /* delete record list start on index page */
#define MLOG_LIST_END_COPY_CREATED                                                                                     \
  ((byte)17)                            /* copy record list end to a new created index page */
#define MLOG_PAGE_REORGANIZE ((byte)18) /* reorganize an index page */
#define MLOG_PAGE_CREATE     ((byte)19) /* create an index page */
#define MLOG_UNDO_INSERT                                                                                               \
  ((byte)20)                           /* insert entry in an undo log */
#define MLOG_UNDO_ERASE_END ((byte)21) /* erase an undo log page end */
#define MLOG_UNDO_INIT                                                                                                 \
  ((byte)22) /* initialize a page in an undo log */
#define MLOG_UNDO_HDR_DISCARD                                                                                          \
  ((byte)23) /* discard an update undo log header */
#define MLOG_UNDO_HDR_REUSE                                                                                            \
  ((byte)24)                            /* reuse an insert undo log header */
#define MLOG_UNDO_HDR_CREATE ((byte)25) /* create an undo log header */
#define MLOG_REC_MIN_MARK                                                                                              \
  ((byte)26) /* mark an index record as the predefined minimum record */
#define MLOG_IBUF_BITMAP_INIT                                                                                          \
  ((byte)27)                      /* initialize an ibuf bitmap page */
...
void mtr_log_write_backup_entries(
    /*=========================*/
    mtr_t *mtr,         /* in: mini-transaction */
