                        ...
                        lsn */
{
  ...
  if (log_do_write) {
    ...
    fil_io(OS_FILE_WRITE | OS_FILE_LOG, sync, group->space_id, dest_offset / UNIV_PAGE_SIZE,
...
loop:
  ...
  if (log_do_write) {
    ...
    fil_io(OS_FILE_WRITE | OS_FILE_LOG, sync, group->space_id, next_offset / UNIV_PAGE_SIZE,
...
loop:
  ...
  if (log_debug_writes) {
    printf("Flushing log from %lu %lu up to lsn %lu %lu\n", ut_dulint_get_high(log_sys->written_to_all_lsn),
           ut_dulint_get_low(log_sys->written_to_all_lsn), ut_dulint_get_high(log_sys->lsn),
                        ...
                        made to log files */
{
  ...
  if (log_debug_writes) {
    printf("Making checkpoint no %lu at lsn %lu %lu\n", ut_dulint_get_low(log_sys->next_checkpoint_no),
           ut_dulint_get_high(oldest_lsn), ut_dulint_get_low(oldest_lsn));
  }
...
loop:
  ...
  fil_io(OS_FILE_READ | OS_FILE_LOG, sync, group->space_id, source_offset / UNIV_PAGE_SIZE,
         source_offset % UNIV_PAGE_SIZE, len, buf, &log_archive_io);
...
loop:
  ...
  if (log_debug_writes) {
    printf("Archiving starting at lsn %lu %lu, len %lu to group %lu\n", ut_dulint_get_high(start_lsn),
           ut_dulint_get_low(start_lsn), len, group->id);
  }
...
loop:
  ...
  if (log_debug_writes) {
    printf("Archiving from lsn %lu %lu to lsn %lu %lu\n", ut_dulint_get_high(log_sys->archived_lsn),
           ut_dulint_get_low(log_sys->archived_lsn), ut_dulint_get_high(limit_lsn), ut_dulint_get_low(limit_lsn));
  }
    ...
    dulint buf_start_lsn) /* in: buffer start lsn */
{
  ...
  recv_scan_log_recs(FALSE, scan_buf, end - start, ut_dulint_align_down(buf_start_lsn, OS_FILE_LOG_BLOCK_SIZE),
...
/*===========*/
{
  ...
  printf("Log sequence number %lu %lu\n"
         ...
         ut_dulint_get_high(log_sys->lsn), ut_dulint_get_low(log_sys->lsn),
         ut_dulint_get_high(log_sys->written_to_some_lsn), ut_dulint_get_low(log_sys->written_to_some_lsn),
         ut_dulint_get_high(log_sys->last_checkpoint_lsn), ut_dulint_get_low(log_sys->last_checkpoint_lsn));

  current_time = time(NULL);

  time_elapsed = difftime(current_time, log_sys->last_printout_time);

  printf("%lu pending log writes, %lu pending chkp writes\n"
         "%lu log i/o's done, %.2f log i/o's/second\n",
         log_sys->n_pending_writes, log_sys->n_pending_checkpoint_writes, log_sys->n_log_ios,
