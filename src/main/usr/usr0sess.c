...
message. */
static void sess_srv_msg_send_error(
    /*====================*/
...
static sess_t *sess_cli_msg_get_sess(
    /*==================*/
    ...
    ulint len) /* in: message string length */
{
  ...
  HASH_SEARCH(hash, sess_sys->hash, fold, sess, UT_DULINT_EQ(id, sess->id));
  return (sess);
}
...
void sess_sys_init_at_db_start(void)
/*===========================*/
{
...
UNIV_INLINE
dulint sess_cli_msg_get_msg_no(
    /*====================*/
    /* out: message number */
    byte *str) /* in: message string */
{
...
UNIV_INLINE
ulint sess_cli_msg_get_cont_size(
    /*=======================*/
...
UNIV_INLINE
ibool sess_cli_msg_check_consistency(
    /*===========================*/
    /* out: TRUE if ok */
    byte *str, /* in: message string */
    ulint len) /* in: message string length */
    ...
    sess_t *sess) /* in, own: session object */
{
  ...
  if (sess->disconnecting && (UT_LIST_GET_LEN(sess->graphs) == 0) && (sess->refer_count == 0)) {
    sess_close(sess);
...
static ulint sess_srv_msg_send_low(
    /*==================*/
                   ...
                   SESS_NOT_RELEASE_KERNEL */
{
  ...
  ut_ad((rel_ker == SESS_NOT_RELEASE_KERNEL) || (rel_ker == SESS_RELEASE_KERNEL));
  ut_ad(mutex_own(&kernel_mutex));
  ...
  ret = com_sendto(sess->endpoint, buf, len, sess->addr_buf, sess->addr_len);
  if (rel_ker == SESS_RELEASE_KERNEL) {
...
static ulint sess_srv_msg_send(
    /*==============*/
    /* out: 0 if success, else error number */
    sess_t *sess,  /* in: session object */
    byte *buf,     /* in: message buffer */
    ...
    sess_t *sess) /* in: session object */
{
  ...
  max_len = ut_min(SESS_ERR_BUF_SIZE, com_endpoint_get_max_size(sess->endpoint));
  ...
  ret = sess_srv_msg_send_low(sess, buf, SESS_SRV_MSG_DATA + err_len, SESS_NOT_RELEASE_KERNEL);
}
                   ...
                   string! */
{
  ...
  trx_sig_send(sess->trx, TRX_SIG_ERROR_OCCURRED, TRX_SIG_SELF, FALSE, NULL, NULL, NULL);
}

/*************************************************************************
Raises an SQL error. */

void sess_raise_error_low(
    /*=================*/
...
static ibool sess_receive_msg_part(
    /*==================*/
    ...
    ulint len)    /* in: message length */
{
  ...
  if (cont == SESS_MSG_FIRST_PART) {
    ...
    ut_memcpy(sess->big_msg + sess->big_msg_len, str + SESS_CLI_MSG_DATA, len - SESS_CLI_MSG_DATA);
...
static void sess_receive_prepare(
    /*=================*/
    sess_t *sess,  /* in: session */
    ...
    ulint len)     /* in: message length */
{
  ...
  sess_srv_msg_send(sess, msg, SESS_SRV_MSG_DATA + 4 + len, SESS_RELEASE_KERNEL);
  sess_refer_count_dec(sess);
}
...
static void sess_receive_command(
    /*=================*/
    sess_t *sess,  /* in: session */
    byte *cli_msg, /* in: client message */
    ulint len,     /* in: message length */
    ulint type)    /* in: message type */
{
  ...
  if (graph->fork_type == QUE_FORK_PROCEDURE_CALL) {
    ...
    graph = dict_procedure_reserve_parsed_copy(call_node->procedure_def);
    graph->trx = sess->trx;

    /* Retrieve the procedure input parameters from the message */

    pars_proc_read_input_params_from_buf(graph, cli_msg + SESS_CLI_MSG_DATA + 4);
    mutex_enter(&kernel_mutex);
  } else {
    ...
    dict_proc = dict_mem_procedure_create(proc_node->proc_id->name, proc_node->sym_tab->sql_string, graph);

    dict_procedure_add_to_cache(dict_proc);

    mutex_enter(&kernel_mutex);

    sess_srv_msg_send_simple(sess, SESS_SRV_SUCCESS, SESS_RELEASE_KERNEL);
    return;
  }
...
void sess_command_completed_message(
    /*===========================*/
    sess_t *sess, /* in: session */
    ...
    ulint len)    /* in: message data length */
{
  ...
  sess_srv_msg_send(sess, msg, SESS_SRV_MSG_DATA + len, SESS_RELEASE_KERNEL);
  mutex_exit(&kernel_mutex);
}

/***************************************************************************
Processes a break message from the client. */
static void sess_receive_break(
    /*===============*/
    sess_t *sess) /* in: session */
{
  ut_ad(mutex_own(&kernel_mutex));
...
void sess_receive_msg_rel_kernel(
    /*========================*/
    sess_t *sess, /* in: session */
    byte *str,    /* in: message string */
    ulint len)    /* in: message length */
...
static ibool sess_open_connection(
    /*=================*/
    byte *str,  /* in: message string */
    ...
    ulint alen) /* in: user address length */
{
  ...
  if (!(UT_DULINT_EQ(sess_id, ut_dulint_zero)) || !(sess_cli_msg_get_type(str) == SESS_CLI_CONNECT)) {
  ...
  sess_srv_msg_send_simple(sess, SESS_SRV_ACCEPT_CONNECT, SESS_NOT_RELEASE_KERNEL);
  return (TRUE);
}
