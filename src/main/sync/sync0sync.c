...
ulint sync_gnuc_intelx86_test_and_set(
                              ...
                              allowed */
{
  ...
  for (i = 0; i < SYNC_THREAD_N_LEVELS; i++) {
    ...
    if (slot->latch != NULL &&
        (!dict_mutex_allowed ||
         (slot->level != SYNC_DICT && slot->level != SYNC_FOREIGN_KEY_CHECK && slot->level != SYNC_PURGE_IS_RUNNING))) {
...
void sync_print_wait_info(void)
/*======================*/
{
#ifdef UNIV_SYNC_DEBUG
  printf("Mutex exits %lu, rws exits %lu, rwx exits %lu\n", mutex_exit_count, rw_s_exit_count, rw_x_exit_count);
#endif
  printf("Mutex spin waits %lu, rounds %lu, OS waits %lu\n"
         "RW-shared spins %lu, OS waits %lu; RW-excl spins %lu, OS waits %lu\n",
         mutex_spin_wait_count, mutex_spin_round_count, mutex_os_wait_count, rw_s_spin_wait_count, rw_s_os_wait_count,
