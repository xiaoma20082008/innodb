  ...
  if (low_match == dtuple_get_n_fields(entry)) {
    ...
    if (rec == NULL) {
      ...
      if (!page_cur_tuple_insert(&page_cur, entry, mtr)) {
        printf("Ibuf insert fails; page free %lu, dtuple size %lu\n", page_get_max_insert_size(page, 1),
...
#ifdef UNIV_IBUF_DEBUG
      volume += rec_get_converted_size(entry) + page_dir_calc_reserved_space(1);
...
/*============*/
{
  ...
  while (data) {
    printf("Ibuf for space %lu: size %lu, free list len %lu, seg size %lu,\n", data->space, data->size,
           data->free_list_len, data->seg_size);
