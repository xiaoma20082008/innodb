...
void trx_sys_doublewrite_restore_corrupt_pages(void)
/*===========================================*/
{
  byte *buf;
  byte *read_buf;
    ...
    mtr_t *mtr) /* in: mtr */
{
  ...
  if (ut_strlen(trx->mysql_log_file_name) > TRX_SYS_MYSQL_LOG_NAME_LEN - 1) {
    ut_memcpy(namebuf,
              trx->mysql_log_file_name + ut_strlen(trx->mysql_log_file_name) - (TRX_SYS_MYSQL_LOG_NAME_LEN - 1),
              TRX_SYS_MYSQL_LOG_NAME_LEN - 1);
  } else {
    ut_memcpy(namebuf, trx->mysql_log_file_name, 1 + ut_strlen(trx->mysql_log_file_name));
  }
  ...
  if (mach_read_from_4(sys_header + TRX_SYS_MYSQL_LOG_INFO + TRX_SYS_MYSQL_LOG_MAGIC_N_FLD) !=
    ...
    mlog_write_ulint(sys_header + TRX_SYS_MYSQL_LOG_INFO + TRX_SYS_MYSQL_LOG_MAGIC_N_FLD, TRX_SYS_MYSQL_LOG_MAGIC_N,
                     MLOG_4BYTES, mtr);
  }

  if (0 !=
      ut_memcmp(sys_header + TRX_SYS_MYSQL_LOG_INFO + TRX_SYS_MYSQL_LOG_NAME, namebuf, TRX_SYS_MYSQL_LOG_NAME_LEN)) {

    mlog_write_string(sys_header + TRX_SYS_MYSQL_LOG_INFO + TRX_SYS_MYSQL_LOG_NAME, namebuf, TRX_SYS_MYSQL_LOG_NAME_LEN,
                      mtr);
  }

  if (mach_read_from_4(sys_header + TRX_SYS_MYSQL_LOG_INFO + TRX_SYS_MYSQL_LOG_OFFSET_HIGH) > 0 ||
      (trx->mysql_log_offset >> 32) > 0) {

    mlog_write_ulint(sys_header + TRX_SYS_MYSQL_LOG_INFO + TRX_SYS_MYSQL_LOG_OFFSET_HIGH,
                     (ulint)(trx->mysql_log_offset >> 32), MLOG_4BYTES, mtr);
  }

  mlog_write_ulint(sys_header + TRX_SYS_MYSQL_LOG_INFO + TRX_SYS_MYSQL_LOG_OFFSET_LOW,
                   (ulint)(trx->mysql_log_offset & 0xFFFFFFFF), MLOG_4BYTES, mtr);
...
/*===================================*/
{
  ...
  fprintf(stderr, "InnoDB: Last MySQL binlog file offset %lu %lu, file name %s\n",
...
/*==========================*/
{
  ...
  trx_sys->max_trx_id =
      ut_dulint_add(ut_dulint_align_up(mtr_read_dulint(sys_header + TRX_SYS_TRX_ID_STORE, MLOG_8BYTES, &mtr),
  ...
  if (UT_LIST_GET_LEN(trx_sys->trx_list) > 0) {
    fprintf(stderr, "InnoDB: %lu uncommitted transaction(s) which must be rolled back\n",
